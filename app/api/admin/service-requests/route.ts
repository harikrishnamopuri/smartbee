...
export async function GET(request: Request) {
  try {
    ...
    const q = adminDb.collection('serviceRequests').orderBy('createdAt', 'desc').offset(offset).limit(pageSize);
    const snap = await q.get();
    const items = snap.docs.map((d: any) => ({ id: d.id, ...(d.data ? d.data() : d.data) }));

    if (format === 'csv') {
      const fields = ['createdAt', 'email', 'uid', 'company', 'service', 'message'];
      ...
      return new Response(csv, { headers: { 'Content-Type': 'text/csv', 'Content-Disposition': 'attachment; filename="service-requests.csv"' } });
    }
